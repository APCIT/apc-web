...
/** Start hour: 4–19 (24h), 12h display. */
...
function TimeEditContent() {
  ...
  useEffect(() => {
    if (!id || !date) {
      setLoadError(!date ? "Missing date parameter." : "Invalid intern id.");
      setLoading(false);
      return;
    }
    GET_INTERN_DETAIL_API(id, date)
      .then((res) => {
        if (!res.ok) {
        ...
        const tl = res.data.thisWeek.find((t) => t.start.slice(0, 10) === date);
        if (!tl) {
          setLoadError("Timelog not found for this date.");
          return;
        }
        setTimelogId(tl.id);
        const s = new Date(tl.start);
        const e = new Date(tl.end);
        setStartHour(String(s.getUTCHours()).padStart(2, "0"));
        setStartMinute(String(s.getUTCMinutes()).padStart(2, "0"));
        setEndHour(String(e.getUTCHours()).padStart(2, "0"));
        setEndMinute(String(e.getUTCMinutes()).padStart(2, "0"));
        setLunch(String(Math.round(tl.lunch * 60)));
        setDescription(tl.description ?? "");
      })
      .catch(() => setLoadError("Failed to load timelog"))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const res = await UPDATE_TIMELOG_API(id, {
      timelogId,
      date,
      startTime,
      endTime,
  ...
  async function handleDelete() {
    if (!window.confirm("Are you sure you want to delete this timelog?")) return;
    setError(null);
    setDeleting(true);
    const res = await DELETE_TIMELOG_API(id, date);
    setDeleting(false);
    if (res.ok) {
      const weekSunday = startOfWeekStr(date);
      router.push(`/Interns/Details/${encodeURIComponent(id)}?date=${weekSunday}`);
    ...
    return (
      <div className="w-full bg-white">
        <div className="py-8 px-[50px]">
          ...
          <div className="text-center mb-6" style={{ color: "#b91c1c" }}>{loadError}</div>
          <div>
            <Link
      ...
      <div className="py-8 px-[50px]">
        ...
        <div className="mb-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            >
              {internName ? `${internName} - Edit Timelog` : "Edit Timelog"}
            </h2>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl">
          ...
          <div className="flex items-start" style={rowStyle}>
            ...
            <div className="flex-1" style={{ marginLeft: "20px" }}>
              {loading ? (
                <div style={{ ...skeletonStyle, height: "96px", width: "400px" }} aria-hidden />
              ) : (
                <textarea
                  id="description"
                  value={description}
          ...
          <div className="flex items-center mt-4 gap-4 flex-wrap">
            ...
            >
              ...
              <button
                ...
                className="inline-flex items-center px-[16px] py-[8px] rounded-[10px] text-white text-[14px] font-normal text-center cursor-pointer select-none border border-[#8b1538] bg-[#9E1B32] hover:bg-[#8b1538] disabled:opacity-70"
              >
                {deleting ? "Deleting…" : "Delete Timelog"}
              </button>
              {error && (
